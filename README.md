...
학부모가 자녀의 학원비 결제 요청을 실시간 푸시 알림으로 받고,
앱 내에서 즉시 결제 및 취소를 처리할 수 있는 서비스입니다.

🏗 프로젝트 디렉토리 구조 (Directory Structure)

프로젝트는 MVVM 아키텍처 기반 기능별 패키징 구조입니다.
...
Push: Firebase Cloud Messaging (Data-only Payload)
...
로그인 성공 시 서버에서 parentId 발급

토큰 자동 등록

로그인 직후 FCM 토큰을 서버 DB에 parentId와 매핑
...
notification 필드 없이 data 필드만 포함
...
receiptId를 Intent로 전달하여 ReceiptDetailActivity 실행
...
object ApiConfig {
    const val API_BASE_URL = "https://qoocca-teachers.r-e.kr"
}
2️⃣ Firebase 설정
...
앱 실행 시 POST_NOTIFICATIONS 권한을 반드시 허용해야 푸시 수신이 가능합니다.
